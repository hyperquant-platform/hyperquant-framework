...
from hyperquant.utils.time_util import get_timestamp_ms, get_timestamp_s
...
class Platform:
    ...
    _internal_id_by_name = {v.upper(): k for k, v in internal_name_by_id.items()}
    ...
    def convert_to_platform_ids(cls, platforms):
        ...
        if isinstance(platforms, int):
            platforms = [platforms]
        return (
            [Platform.get_platform_id_by_name(p) for p in platforms]
            if platforms
            else []
        )
...
class PlatformCredentials:
    ...
    template_by_id = {
        ...
        Platform.BINANCE_FUTURE: common,
    ...
    def to_common_with_passphrase(cls, credentials) -> tuple:
        ...
        if isinstance(credentials, Mapping):
            return tuple(credentials.get(key) for key in cls.common_with_passphrase)
        elif isinstance(credentials, Iterable) and not isinstance(credentials, str):
...
class Market:
    ...
    currency_regexps_by_market_by_platform_id = {
        "default": {USD: ["USD", ".USD", "USD.", "PAX"], ALL: [".+"]},
        # Platform.BINANCE: {
        #     USD: ["USD", ".USD", "USD.", "PAX"],
        # },
    }
    ...
    def get_regexp_by_market_lookup(cls, platform_id):
        ...
        result = {
            market: cls._market_to_regexp(platform_id, market) for market in markets
        }
        return result
    ...
    def _market_to_regexp(cls, platform_id, market):
        ...
        if market == cls.ALTERNATIVES:
            ...
            except_regexps = [
                cls._market_to_regexp(platform_id, m) for m in markets if m != market
            ]
            ...
            currency_regexps = (
                currency_regexps_by_market.get(
                    market, currency_regexps_by_market.get("default", [market])
                )
                if currency_regexps_by_market
                else [market]
            )
...
class Endpoint:
    ...
    )  # For BitMEX (deposits, withdrawals, realisedPNLs)
    ...
    def convert_to_endpoints(cls, endpoints):
        ...
        return (
            [e.lower() for e in endpoints if e.lower() in cls.ALL] if endpoints else []
        )
...
class Currency(Symbol):
    ...
    def convert_symbols(cls, symbols):
        ...
        if isinstance(symbols, str):
            ...
            return (
                symbols.upper()
                .replace("-", Currency.DELIMITER)
                .replace("/", Currency.DELIMITER)
            )
        # list -> list
        return [
            s.upper().replace("-", Currency.DELIMITER).replace("/", Currency.DELIMITER)
            for s in symbols
            if s
        ]
...
class CurrencyPair(Symbol):
    ...
    def get_canonical_name(self):
        return (
            ...
            and self.quote
            and "{}:{}{}".format(
                self.name_in_platform, self.base.upper(), self.quote.upper() or ""
            )
        )
...
class CandleInterval:
            ...
            for v in intervals.values():
                result.update(
                    set(
                        ...
                        else cls.convert_to_intervals(v)
                    )
                )
        else:
    ...
    def sorted(cls, candle_intervals):
        ...
        return sorted(candle_intervals, key=lambda i: cls.convert_to_minutes(i) or -1)
    ...
    PROPERTIES_BY_SUFFIX = {
        ...
        "Y": ["year", "month", "day", "hour", "minute", "second", "microsecond"],
    }
    # month = 1 .. 12
    # day = 1 ..
    ...
    def break_time_period_on_intervals(
        cls,
        from_incl,
        to_excl,
        candle_interval,
        ...
        from_datetime = datetime.utcfromtimestamp(get_timestamp_s(from_incl)).replace(
            tzinfo=timezone.utc
        )
        to_datetime = datetime.utcfromtimestamp(get_timestamp_s(to_excl)).replace(
            tzinfo=timezone.utc
        )
        ...
        to_datetime = cls._get_side_datetime_for_interval(
            to_datetime,
            interval_value,
            interval_suffix,
            weekday_start,
        ...
        while True:
            ...
            if is_return_timestamps:
                if is_milliseconds:
                    ...
                    result.append(
                        [from_datetime.timestamp(), current_to_datetime.timestamp()]
                    )
            else:
    ...
    @classmethod
    def _reset_datetime_for_interval_suffix(cls, for_datetime, interval_suffix):
        ...
        side_datetime = for_datetime.replace(
            **{p: cls.MIN_VALUE_BY_PROPERTY.get(p) for p in zero_properties}
        )
    ...
    def _get_max_datetime(cls, for_datetime, interval_suffix):
        ...
        if properties:
            ...
            delta_property = (
                "days" if period_property == "week" else period_property + "s"
            )
            delta = relativedelta(**{delta_property: 1})
            # max_datetime result
            max_datetime = (
                cls._reset_datetime_for_interval_suffix(for_datetime, period_suffix)
                + delta
            )
            return max_datetime

    @classmethod
    def _get_side_datetime_for_interval(
        cls,
        for_datetime,
        interval_value,
        interval_suffix,
        ...
        current_value = (
            for_datetime.weekday()
            if property == "week"
            else getattr(for_datetime, property)
        )
        ...
        if interval_value > 1:
            ...
            delta_value = (
                interval_value - (current_value - min_value) % interval_value
            ) % interval_value
            if is_before and delta_value:
                delta_value -= interval_value
            delta = relativedelta(**{property + "s": delta_value})
            side_datetime += delta
        return min(side_datetime, max_datetime) if max_datetime else side_datetime

    @classmethod
    def get_seconds_from_interval(self, candle_interval):
        interval_size, suffix = CandleInterval.parse_interval(candle_interval, "m")
...
class Direction:
    ...
    name_by_value = {SELL: SELL_NAME, BUY: BUY_NAME}
    ...
    def get_direction_value(cls, direction, is_check_valid_id=True):
        return cls.value_by_name.get(
            str(direction).upper(),
            direction
            if not is_check_valid_id or direction in cls.name_by_value
            else None,
        )
...
class OrderBookAggType:
    ...
    ALL = [SIMPLE, ADVANCED, WEIGHTED_ARITHMETIC, WEIGHTED_GEOMETRIC, WEIGHTED_HARMONIC]
...
class OrderBookDirection:
    ...
    name_by_value = {ASK: ASK_NAME, BID: BID_NAME}
    value_by_name = {v: k for k, v in name_by_value.items()}
...
class OrderStatus:
    ...
    OPEN = (
        1
    )  # When we cannot convert receiving status precisely to NEW or PARTIALLY_FILLED
...
class ErrorCode:
    ...
    def get_message_by_code(cls, code, default=None, **kwargs):
        return (
            cls.message_by_code[code].format_map(kwargs)
            if code in cls.message_by_code
            else default or "(no message: todo)"
        )
...
class DateTimeFormat:
    ...
    name_by_value = {TIMESTAMP: TIMESTAMP_NAME, DATETIME: DATETIME_NAME, ISO: ISO_NAME}
    value_by_name = {v: k for k, v in name_by_value.items()}
...
class FieldSeparator:
    ...
    name_by_value = {COMMA: COMMA_NAME, SPACE: SPACE_NAME, SEMICOLON: SEMICOLON_NAME}
    value_by_name = {v: k for k, v in name_by_value.items()}
...
class FractionalSeparator:
    ...
    name_by_value = {DOT: DOT_NAME, COMMA: COMMA_NAME}
    value_by_name = {v: k for k, v in name_by_value.items()}
...
class FileFormat:
    ...
    name_by_value = {CSV: CSV_NAME, TXT: TXT_NAME, JSON: JSON_NAME}
    value_by_name = {v: k for k, v in name_by_value.items()}
...
class LoadingStatus:
    ...
    name_by_value = {ACTIVE: ACTIVE_NAME, STOPPED: STOPPED_NAME, ERROR: ERROR_NAME}
    value_by_name = {v: k for k, v in name_by_value.items()}
...
class TransactionType:
    ...
    DIRECT = [DEPOSIT, WITHDRAWAL, TRANSFER, AFFILIATE_PAYOUT, FUNDING, REFERRAL_PAYOUT]
    ALL = [DEPOSIT, WITHDRAWAL, REALISED_PNL, UNREALISED_PNL, TRANSFER, AFFILIATE_PAYOUT, FUNDING, REFERRAL_PAYOUT]

    @classmethod
    def check_is_direct(cls, type):
        return type in cls.DIRECT
...
item_format_by_endpoint = {
    ...
    Endpoint.TICKER_24H: [
        ...
        ParamName.PRICE_HIGH,
        ParamName.PRICE_LOW,
        ParamName.PRICE_LAST,
        ParamName.PRICE_FIRST,
        ParamName.PRICE_CHANGE_PERCENT,
        ParamName.VOLUME,
    ],
    Endpoint.QUOTE: [
        ParamName.PLATFORM_ID,
    ...
    #     ParamName.PRICE, ParamName.AMOUNT, ParamName.DIRECTION
    # ],
    Endpoint.ACCOUNT: [ParamName.PLATFORM_ID, ParamName.TIMESTAMP],
    Endpoint.BALANCE: [
        ParamName.PLATFORM_ID,
...
name_by_value_by_param_name = {
    ...
    ParamName.DIRECTION: Direction.name_by_value,  # OrderBookDirection.name_by_value],
...
parse_bool_info = (
    "True, true, TRuE, t, 1, 123, y, yes - for true; "
...
def parse_bool(value, default=None):
    ...
    return (
        bool(
            value.lower() in ("true", "t", "1", "y", "yes")
            or (value.isnumeric() and int(value))
        )
...
def parse_list(params, *names):
    ...
    if (
        not params
        or not names
        or not any(params.get(name, None) for name in names if params.get(name, None))
    ):
        return None
    items = next(
        params.getlist(name) if hasattr(params, "getlist") else params.get(name, None)
        for name in names
...
def parse_platform_ids(params):
    platforms = parse_list(
        params,
        ParamName.PLATFORM_ID,
        ParamName.PLATFORM_IDS,
        ParamName.PLATFORMS,
        ParamName.PLATFORM,
    )
    return (
        [_convert_platform_id(p) for p in platforms]
...
def parse_direction(params, is_remove_wrong_value=True):
    ...
    direction = (
        ...
        else Direction.value_by_name.get(direction.lower(), direction)
    )
    return (
        direction
        if not is_remove_wrong_value or direction in (Direction.SELL, Direction.BUY)
        else None
    )
...
def parse_timestamp(params, name):
    ...
    return get_timestamp_ms(time)


def parse_datetime(params, name):
    # Any time value to Unix timestamp in seconds
    time = params.get(name) if params else None
    timestamp_s = get_timestamp_s(time)
    result = datetime.utcfromtimestamp(timestamp_s) if timestamp_s is not None else None
    return result
...
def parse_limit_and_limit_skip(params, DEFAULT_LIMIT, MIN_LIMIT, MAX_LIMIT):
    limit = params.get(ParamName.LIMIT, DEFAULT_LIMIT) if params else DEFAULT_LIMIT
    ...
    if not limit_skip or (isinstance(limit_skip, str) and not limit_skip.isnumeric()):
        limit_skip = 0
    else:
...
def parse_sorting(params, DEFAULT_SORTING):
    sorting = (
        params.get(ParamName.SORTING, DEFAULT_SORTING) if params else DEFAULT_SORTING
    )
    return sorting
...
def convert_items_obj_to_list(item_or_items, item_format):
    ...
    return _convert_item_or_items_with_fun(
        item_or_items, item_format, _convert_items_obj_to_list
    )
...
def convert_items_dict_to_list(item_or_items, item_format):
    if not item_or_items:
        return item_or_items
    return _convert_item_or_items_with_fun(
        item_or_items, item_format, _convert_items_dict_to_list
    )
...
def convert_items_to_list(item_or_items, item_format):
    if not item_or_items:
        return item_or_items
    return _convert_item_or_items_with_fun(
        item_or_items, item_format, _convert_items_to_list
    )
...
def convert_items_list_to_dict(item_or_items, item_format):
    if not item_or_items:
        return item_or_items
    return _convert_item_or_items_with_fun(
        item_or_items, item_format, _convert_items_list_to_dict
    )
...
def convert_items_obj_to_dict(item_or_items, item_format):
    if not item_or_items:
        return item_or_items
    return _convert_item_or_items_with_fun(
        item_or_items, item_format, _convert_items_obj_to_dict
    )
...
def convert_items_to_dict(item_or_items, item_format):
    if not item_or_items:
        return item_or_items
    return _convert_item_or_items_with_fun(
        item_or_items, item_format, _convert_items_to_dict
    )
...
def convert_items_to_obj(item_or_items, item_format, item_type):
    if not item_or_items:
        return item_or_items
    return _convert_item_or_items_with_fun(
        item_or_items, item_format, _convert_items_to_obj, item_type
    )
        ...
        for element in item_or_items:
            if element:
                ...
                if (
                    ...
                    or not isinstance(element, Iterable)
                    and not hasattr(element, "__dict__")
                ):
                    is_list = False
                break
...
def _convert_items_obj_to_list(items, item_format):
    return (
        [
            [getattr(item, p) for p in item_format if hasattr(item, p)]
            if item is not None
            else None
            for item in items
        ]
...
def _convert_items_dict_to_list(items, item_format):
    return (
        [
            [item[p] for p in item_format if p in item] if item is not None else None
            for item in items
        ]
        if items
        else []
...
def _convert_items_list_to_dict(items, item_format):
    ...
    return (
        [
            {p: item[i] for i, p in index_property_list if i < len(item)}
            if item is not None
            else None
            for item in items
        ]
...
def _convert_items_obj_to_dict(items, item_format):
    return (
        [
            {p: getattr(item, p) for p in item_format if hasattr(item, p)}
            if item is not None
            else None
            for item in items
        ]
...
def _convert_items_list_to_obj(items, item_format, obj_type):
    ...
    for item in items:
        ...
        for i, p in enumerate(item_format):
            ...
            value = (
                Decimal(item[i])
                if ParamName.is_decimal(p) and item[i] is not None
                else item[i]
            )
            setattr(obj, p, value)
        if (
            hasattr(obj, "is_milliseconds")
            and hasattr(obj, ParamName.TIMESTAMP)
            and obj.timestamp
        ):
