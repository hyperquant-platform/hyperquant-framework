...
from hyperquant.clients import (Account, Balance, Candle, Endpoint, Error,
                                ...
                                ParamName, PrivatePlatformRESTClient, Quote,
...
class BinanceRESTConverterV1(RESTConverter):
    ...
    param_value_lookup = {
        ...
        # By properties:
        ParamName.DIRECTION: {Direction.SELL: "SELL", Direction.BUY: "BUY"},
        ...
        ParamName.LEVEL: {
            OrderBookDepthLevel.LIGHT: 100,
            OrderBookDepthLevel.MEDIUM: 500,
            OrderBookDepthLevel.DEEP: 1000,
            OrderBookDepthLevel.DEEPEST: 5000,
        }
    }
    ...
    max_limit_by_endpoint = {
        ...
        Endpoint.ORDER: 1000,
        Endpoint.ORDERS_ALL: 1000,
        Endpoint.TRADE_MY: 1000,
        Endpoint.TRADE_MY_HISTORY: 1000,
    }
    ...
    param_lookup_by_class = {
        ...
        Ticker: {"symbol": ParamName.SYMBOL, "price": ParamName.PRICE},
        Account: {
    ...
    def __init__(self, platform_id=None, version=None):
        self.intervals_supported = self.param_value_lookup[ParamName.INTERVAL].keys()
        super().__init__(platform_id=platform_id, version=version)

    def _process_param_value(self, name, value):
        if name == ParamName.FROM_ITEM or name == ParamName.TO_ITEM or name == 'orderId':
    ...
    def _parse_item(self, endpoint, item_data):
        ...
        self._parse_my_trade_direction(result)
        return result

    def _parse_my_trade_direction(self, item):
        # MyTrade specific direction param
        if item and isinstance(item, MyTrade) and hasattr(item, "direction"):
            item.direction = (
                Direction.BUY
                if item.direction is True
                else (Direction.SELL if item.direction is False else None)
            )
    ...
    def parse(self, endpoint, data):
        ...
        if data:
            if endpoint == Endpoint.SERVER_TIME:
                ...
                return (
                    timestamp_ms / 1000
                    if not self.use_milliseconds and timestamp_ms
                    else timestamp_ms
                )
            elif (
                endpoint in [Endpoint.SYMBOLS, Endpoint.CURRENCY_PAIRS]
                ...
                for item in exchange_info:
                    del item["filters"][5]  # this leaf has duplicated and ==0 lot sizes
                    _item = {}
                    expand_complex(item, _item)
                    symbols.append(
                        super().parse(endpoint, _item)
                        if endpoint == Endpoint.CURRENCY_PAIRS
                        else _item[ParamName.SYMBOL]
                    )
    ...
    def _order_params(self, platform_params):
        ...
        params_list = [
            (key, value) for key, value in platform_params.items() if key != "signature"
        ]
...
class BinanceRESTClient(PrivatePlatformRESTClient):
    ...
    def fetch_trades_history(
        ...
        **kwargs
        ...
        return super().fetch_trades_history(
            symbol,
            limit=limit,
            from_item=from_item,
            to_item=to_item,
    ...
    def fetch_trades(self, symbol, limit=None, version=None, **kwargs):
        return super().fetch_trades(
            symbol, limit=limit, version=version or "1", **kwargs
        )

    def get_server_timestamp(self, force_from_server=False, version=None, **kwargs):
        return super().get_server_timestamp(
            force_from_server=force_from_server, version=version or "1", **kwargs
        )
        ...
        if isinstance(result, Error):
            if result.code == ErrorCode.RATE_LIMIT:
                ...
                self.delay_before_next_request_sec = (
                    60 * 2 * self.ratelimit_error_in_row_count
                )  # some number - change
            elif result.code == ErrorCode.IP_BAN:
                self.ratelimit_error_in_row_count += 1
                self.delay_before_next_request_sec = (
                    60 * 5 * self.ratelimit_error_in_row_count
                )  # some number - change
            else:
    ...
    def fetch_history(
        ...
        **kwargs
        ...
        if from_item is None and endpoint != Endpoint.ORDERS_ALL:
            from_item = 0
        if endpoint in [Endpoint.ORDER, Endpoint.ORDERS_ALL, Endpoint.TRADE_MY, Endpoint.TRADE_MY_HISTORY]:
            version = version or '3'
            if not from_item and not from_time and not to_time:
                # Setting fake first item to load history in ASC
                from_item = ItemObject(platform_id=self.platform_id, timestamp=0, item_id=1)
        if from_item and endpoint == Endpoint.ORDERS_ALL:
            kwargs['orderId'] = from_item
            from_item = None
        return super().fetch_history(endpoint, symbol, limit, from_item,
                                     to_item, sorting, is_use_max_limit,
                                     from_time, to_time, version=version, **kwargs)
    ...
    def fetch_ticker(self, symbol=None, version=None, **kwargs):
        return super().fetch_ticker(symbol=symbol, version=version or "3", **kwargs)
    ...
    def fetch_my_trades(
        ...
        return super().fetch_my_trades(
            symbol, limit, from_item, version or "3", **kwargs
        )
    ...
    def cancel_order(self, order, symbol=None, version=None, **kwargs):
        ...
        if order and not symbol:
            ...
            symbol = [order for order in orders if order.item_id == order_id][0].symbol
    ...
    def fetch_order(self, order_or_id, symbol=None, version=None, **kwargs):
        return super().fetch_order(order_or_id, symbol, version or "3", **kwargs)
    ...
    def fetch_orders(
        ...
        **kwargs
        ...
        result = super().fetch_orders(
            symbol, limit, from_item, False, version or "3", **kwargs
        )
    ...
    def get_positions(self, symbol=None, limit=None, version=None, **kwargs):
        return super().get_positions(
            symbol=symbol, limit=limit, version=version or "3", **kwargs
        )

    def _send(self, method, endpoint, params=None, version=None, **kwargs):
        if endpoint in self.converter.secured_endpoints:
            params[ParamName.TIMESTAMP] = int(time.time() * 1000) - 5000  # Subtruct 5 seconds just to be sure
            params['recvWindow'] = 30000  # 30 sec windows will cover possible lags and 5 sec of subtructed seconds
...
class BinanceWSRestHelper(BinanceRESTClient):
    ...
    base_url = "https://api.binance.com/api/v1/userDataStream"
    ...
    def stream_keep_alive(self):
        ...
        requests.put(self.base_url, headers=self.headers, data=params)
        self._restart_reconnector()

    def _restart_reconnector(self):
        if self._user_timer is not None:
            # Cancel active timer before creating a new one
...
class BinanceWSConverterV1(WSConverter):
    ...
    def _store_subscription(self, endpoints, symbols=None, **params):
        ...
        iterator = (
            itertools.product(endpoints, symbols, _map)
            if symbols
            else ((e, None, p) for e, p in itertools.product(endpoints, _map))
        )
    ...
    def preprocess_data(self, data, subscription, endpoint, symbol, params):
        ...
        if "data" in data:
            data["data"]["s"] = symbol
        return super().preprocess_data(data, subscription, endpoint, symbol, params)
    ...
    def parse(self, endpoint, data):
        ...
        if isinstance(endpoint, list):
            endpoints = endpoint
            result = [
                super(WSConverter, self).parse(endpoint, data) for endpoint in endpoints
            ]
    ...
    def get_subscription_info(self, endpoint, data):
        ...
        if self.endpoint_symbol_params_by_subscription:
            ...
            if "e" in data and Endpoint.check_is_private(
                list(self.endpoint_symbol_params_by_subscription.values())[0][0]
            ):
            ...
            if len(self.endpoint_symbol_params_by_subscription) == 1:
                ...
                subscription, (endpoint, symbol, params) = list(
                    self.endpoint_symbol_params_by_subscription.items()
                )[0]
            else:
                if not subscription:
...
class BinanceWSClient(WSClient):
    ...
    helper_class = BinanceWSRestHelper
    ...
    def __init__(
        ...
        if any([api_key, api_secret, credentials]):
            if 'pivot_symbol' in kwargs:
                self.pivot_symbol = kwargs['pivot_symbol']
                kwargs.pop('pivot_symbol')
                self.rest_helper = self.helper_class(
                    api_key,
                    api_secret,
                    credentials=credentials,
    ...
    def _parse(self, endpoint, data):
        ...
        if not result:
            return
        data_endpoint = (
            result[0].endpoint if isinstance(result, list) else result.endpoint
        )
        if Endpoint.check_is_private(data_endpoint):
            if isinstance(result, list):
                if data_endpoint == Endpoint.BALANCE:
                    if (
                        Endpoint.POSITION in self.current_subscriptions
                        and Endpoint.BALANCE in self.current_subscriptions
                    ):
                        result.extend(self.make_positions_from_balance(result))
                    elif Endpoint.POSITION in self.current_subscriptions:
                ...
                self.logger.exception(
                    "Unexpected behavior got not a list in private endpoint", result
                )
        else:
            return result
    ...
    def make_positions_from_balance(self, balances):
        ...
        for balance in balances:
            ...
            if position:
                position.subscription = Endpoint.POSITION
